...
contract MusicDistribution is ERC721("DAppFi", "DAPP"), Ownable {
    string public baseURI =
        "https://gateway.pinata.cloud/ipfs/QmWwJgr6y6GSA5nrvxsQHjkDsXnEK6g7zsgjwxHwi8vm2K";
    ...
    function resellSong(uint256 _tokenId, uint256 _price) external payable {
    ...
    function getAllUnsoldSongs() external view returns (MarketItem[] memory) {
    ...
    function getMySongs() external view returns (MarketItem[] memory) {
        uint256 mySongCount = balanceOf(msg.sender);
        MarketItem[] memory tokens = new MarketItem[](mySongCount);
        uint256 currentIndex;
        for (uint256 i = 0; i < marketItems.length; i++) {

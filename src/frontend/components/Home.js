...
const Home = ({ contract }) => {
  ...
  const loadMarketplaceItems = async () => {
    const results = await contract.getAllUnsoldSongs()
    const marketItems = await Promise.all(results.map(async i => {
      const uri = await contract.tokenURI(i.tokenId);
      const correctedUri = uri.slice(0, -1);
  ...
  const buyMarketItem = async (item) => {
    await (await contract.buySong(item.itemId, { value: item.price })).wait()
  ...
  const skipSong = (forwards) => {
    setCurrentItemIndex((index) => {
      ...
      if (index < 0) index = marketItems.length - 1;
      if (index >= marketItems.length) index = 0;
      return index;
    });
  }
        ...
        <main role="main" className="col-lg-12 mx-auto" style={{ maxWidth: '500px' }}>
          <div className="content mx-auto">
            ...
            <Card>
              ...
              <Card.Img variant="top" src={marketItems[currentItemIndex].imageUrl} />
